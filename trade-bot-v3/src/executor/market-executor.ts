/**
 * Executor: GTD limit buy at target price, market sell, get order status, cancel.
 */
...
/** GTD limit buy at target price. Lifetime = GTD_LIFETIME_SEC; expiration = now + GTD_BUFFER_SEC + GTD_LIFETIME_SEC. */
export async function limitBuyGTD(
  tokenId: string,
  amountUsd: number,
  price: number
  ...
  const expiration = Math.floor(Date.now() / 1000) + config.GTD_BUFFER_SEC + config.GTD_LIFETIME_SEC;
  const order = {
    tokenID: tokenId,
    side: Side.BUY,
    size,
