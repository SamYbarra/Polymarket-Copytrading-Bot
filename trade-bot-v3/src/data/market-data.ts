/**
 * Market data: current 5m window, slug, get current market (same pattern as v2).
 */
...
function getMarketInfo(event: GammaEvent): MarketInfo | null {
  ...
  return { conditionId, eventSlug: event.slug || "", startTime, endTime };
}
...
export async function getCurrentMarket(): Promise<{
  ...
  if (!upTokenId || !downTokenId) {
    const market = event.markets.find((x: any) => x.conditionId === marketInfo.conditionId) ?? event.markets[0];
